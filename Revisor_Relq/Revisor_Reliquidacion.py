...
clave_concepto = _texto.clave_mayusculas
...
#  Lectura de valores
# =============================================================================
# Los adaptadores Excel viven en revisor/lectores.py. Se reexportan para
# conservar los nombres históricos que usan los verificadores de este archivo.
try:
    ...
    from revisor.lectores import (
        ...
        leer_celdas_rapido, leer_columna_excel, leer_columnas_rapido, leer_formulas_rapido,
        resolver_hoja, ubicar_hoja_xml,
    )
except ImportError as e:
    _morir_import(
        "No se pudo cargar revisor/lectores.py.\n\n"
        f"Carpeta actual: {DIR_SCRIPT}\n\nDetalle: {e}",
        titulo="Falta la carpeta revisor/")
...
def conexion_mdb(ruta):
    return _lectores.conexion_mdb(ruta)


def listar_tablas_mdb(ruta, log):
    return _lectores.listar_tablas_mdb(ruta, log)


def obtener_tablas_columnas(ruta):
    return _lectores.obtener_tablas_columnas(ruta)


def desglose_por_tipo(ruta, tabla, columna, columna_tipo, where, log):
    return _lectores.desglose_por_tipo(ruta, tabla, columna, columna_tipo, where, log)


def leer_valor_mdb(ruta, tabla, columna, where, log):
    return _lectores.leer_valor_mdb(ruta, tabla, columna, where, log)


def obtener_valor(clave, rutas, log, usar_cache=True):
    return _lectores.obtener_valor(clave, rutas, log, usar_cache)


try:
    from revisor import verificaciones as _verificaciones
    from revisor import lanzamiento as _lanzamiento
except ImportError as e:
    _morir_import(
        "No se pudieron cargar los motores internos del Revisor.\n\n"
        f"Detalle: {e}", titulo="Falta la carpeta revisor/")

# Los módulos reciben las dependencias históricas sin importar este punto de
# entrada; así no hay ciclos y los nombres externos siguen viviendo acá.
_lectores.configurar(globals())
_verificaciones.configurar(globals())
_lanzamiento.configurar(globals())
...
class Revisor:
    ...
    def _bloqueado_para_escritura(self, ruta):
        return _lanzamiento.bloqueado_para_escritura(self, ruta)

    def _lock_excel(self, ruta):
        return _lanzamiento.lock_excel(self, ruta)
    ...
    def _comprobar(self, c, valores, L):
        return _verificaciones.comprobar(self, c, valores, L)
