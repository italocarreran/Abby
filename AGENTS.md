...
| Un `from __comun__ ...` o `from revisor ...` sin `try/except ImportError` | Va **siempre** adentro de la guarda que llama a `_morir()`/`_morir_import()`, y esa función tiene que estar definida antes. Lanzado con `pythonw` no hay consola: un `ImportError` suelto mata el script en silencio y solo se ve que la ventana no aparece. Por eso `_morir()` no puede mudarse a `__comun__/`: es lo que avisa cuando `__comun__/` es lo que falta. **Ya se rompió tres veces.** |
...
| Un ayudante que se muda y su llamador que se queda | Al sacar código a `revisor/*.py`, comprobar que el punto de entrada siga **resolviendo todos los nombres que usa**. `_suma_rango_openpyxl` se fue a `lectores.py` y `leer_valor_excel` se quedó usándolo: como el uso estaba dentro de un `try/except`, no reventaba — caía al camino de Excel por COM. El número salía **bien**, así que no se notaba, pero cada rango abría Excel en la T:, que es justo lo que el camino rápido existe para evitar. Lo cubre `revisor/test_modulos_fase4.py`. |
| Un wrapper que repite el valor por omisión en vez de reenviar el argumento | Al dejar un método delegando en `revisor/*.py`, los argumentos se **reenvían** (`indice=indice`), nunca se reescribe el default (`indice=0`). No falla ni avisa: `_lanzar_actualizador` con `indice=0` fijo hacía que el **segundo botón de cinco filas** lanzara el script de la primera (el botón "Prorratear" de los tres `.mdb` abría el actualizador de data), y `_armar_traspaso` con `nid=None` dejaba el JSON sin `nodo`, que es justo lo que el script necesita para saber desde qué fila lo llamaron. Lo cubre `revisor/test_modulos_fase4.py`. |
| Una constante definida arriba y **también** importada más abajo | El import gana y la definición de arriba queda muerta: editarla no hace nada, en silencio. Le pasó a `TOL_MTIME`, que estaba en el bloque de tolerancias del Revisor y otra vez en `revisor/archivos.py`. Al sacar algo a un módulo, borrar la definición vieja y dejar en su lugar un comentario que diga dónde vive ahora. |
| Extraer código a un módulo y perder los comentarios en el camino | El *qué* se lee del código; el *porqué* no. Al mover `estado.py` se perdieron el `OJO: aca la comparacion es EXACTA, sin la tolerancia de TOL_MTIME` y el porqué del caché de directorios — justo lo que evita que alguien "unifique" las dos comparaciones de fecha y reviva un bug. Los comentarios viajan con el código. |
...
python Revisor_Relq/revisor/test_estado.py   # pruebas internas del Revisor
```
